    ...
    return <AppLayout title="Sankalp Families">
        <div className="grid gap-5 xl:grid-cols-[1fr_390px]">
            <section className="hf-card p-4 md:p-5 overflow-x-auto">
                ...
                <table className="hf-table min-w-[1040px]"><thead><tr><th>Family</th><th>Head / Contact</th><th>Center</th><th>Members</th><th>Area / Society</th><th>Group</th><th>Source</th><th>Status</th></tr></thead><tbody>
            ...
            <section className="hf-card p-5 h-fit">
                ...
                <Form action="/registration/families" method="post" className="space-y-3" transform={data=>({...data, members})}>{({ errors, processing, reset }) => <>
                    ...
                    <div className="grid grid-cols-2 gap-2"><FormField label="Mobile" error={errors.head_mobile}><input name="head_mobile" className="hf-input"/></FormField><FormField label="City / Village"><input name="city_village" className="hf-input"/></FormField></div>
                    ...
                    <div className="border-t border-[#eadff0] pt-3"><div className="flex justify-between items-center mb-2"><span className="font-bold text-sm">Family Members</span><button type="button" className="hf-btn hf-btn-secondary text-xs" onClick={()=>setMembers([...members,newMember()])}><Plus size={14} className="inline"/> Add</button></div>
                        <div className="space-y-3">{members.map((m,i)=><div key={i} className="rounded-xl bg-[#faf7fc] border border-[#eadff0] p-3 space-y-2"><div className="flex gap-2"><input className="hf-input" placeholder="Member name" value={m.name} onChange={e=>setMembers(members.map((x,j)=>j===i?{...x,name:e.target.value}:x))}/>{members.length>1&&<button type="button" onClick={()=>setMembers(members.filter((_,j)=>j!==i))}><Trash2 size={17}/></button>}</div><div className="grid grid-cols-2 gap-2"><select className="hf-input" value={m.gender} onChange={e=>setMembers(members.map((x,j)=>j===i?{...x,gender:e.target.value as 'male'|'female'}:x))}><option value="male">Male</option><option value="female">Female</option></select><input className="hf-input" type="number" min="0" max="120" placeholder="Age" value={m.age} onChange={e=>setMembers(members.map((x,j)=>j===i?{...x,age:e.target.value}:x))}/></div><input className="hf-input" placeholder="Relationship" value={m.relationship} onChange={e=>setMembers(members.map((x,j)=>j===i?{...x,relationship:e.target.value}:x))}/><label className="text-xs flex gap-2"><input type="checkbox" checked={m.is_head} onChange={e=>setMembers(members.map((x,j)=>({...x,is_head:j===i?e.target.checked:false})))}/> Head member</label></div>)}</div>
                    </div>
