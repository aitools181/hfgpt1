...
type FamilyOption={id:number;center_id:number;society_id:number|null;external_family_id:string|null;manual_reference:string|null;head_name:string};
...
export default function BalCompletions({reports,options}:any){
 ...
 const societies=useMemo(()=>options.societies.filter((s:any)=>String(s.center_id)===String(group?.center_id??'')),[options.societies,group?.center_id]);

 useEffect(()=>{
  setFamilies([]); f.setData('family_id',''); setFamilySearch('');
 },[f.data.group_id]);

 useEffect(()=>{
  if(!f.data.group_id){setFamilies([]);return;}
  const controller=new AbortController();
  const timer=window.setTimeout(async()=>{
   setFamilyLoading(true);
   try{
    const p=new URLSearchParams(); if(familySearch.trim())p.set('q',familySearch.trim()); if(f.data.society_id)p.set('society_id',String(f.data.society_id));
    const res=await fetch(`/bal-pravruti/groups/${f.data.group_id}/families/search?${p.toString()}`,{credentials:'same-origin',headers:{Accept:'application/json'},signal:controller.signal});
    if(!res.ok)throw new Error('Family search failed');
    const data=await res.json(); setFamilies(Array.isArray(data.families)?data.families:[]);
   }catch(e:any){if(e?.name!=='AbortError')setFamilies([])}finally{if(!controller.signal.aborted)setFamilyLoading(false)}
  },300);
  return()=>{window.clearTimeout(timer);controller.abort()};
 },[f.data.group_id,f.data.society_id,familySearch]);
 ...
 return <AppLayout title="Bal Pravruti Entry / Completion"><Head title="Bal Completion"/>
  <div className="grid gap-5 2xl:grid-cols-[430px_1fr]">
   <form className="hf-card p-5 space-y-3" onSubmit={e=>{e.preventDefault();if(f.data.group_id)f.post(`/bal-pravruti/groups/${f.data.group_id}/completions`,{onSuccess:()=>{f.reset('society_id','family_id','mobile','family_name','family_details');setFamilies([]);setFamilySearch('')}})}}><div><div className="text-xs font-bold uppercase tracking-[.14em] text-[#7b5f87]">Assigned Sanchalak only</div><h2 className="text-lg font-extrabold">Submit Family Completion Report</h2></div>
    ...
    <Field label="Find Sankalp Family (optional)"><input className="hf-input" value={familySearch} onChange={e=>setFamilySearch(e.target.value)} disabled={!f.data.group_id} placeholder={f.data.group_id?'Type Family ID or Head name':'Select Group first'}/></Field>
    <Field label="Known Sankalp Family (optional)"><select className="hf-input" value={f.data.family_id} onChange={e=>f.setData('family_id',e.target.value)} disabled={!f.data.group_id||familyLoading}><option value="">{familyLoading?'Searching...':'Not linked / enter details below'}</option>{families.map(x=><option key={x.id} value={x.id}>{x.external_family_id??x.manual_reference} - {x.head_name}</option>)}</select><div className="mt-1 text-xs text-[#75667a]">Search returns up to 30 matching active Families instead of loading the full Center database.</div></Field>
   ...
   <section className="hf-card p-5 overflow-x-auto"><h2 className="font-extrabold">My Bal Completion History</h2><div className="hf-table-scroll"><table className="hf-table hf-mobile-table mt-3"><thead><tr><th>Date</th><th>Group</th><th>Society</th><th>Family</th><th>Visited</th><th>Completed</th><th>Mobile</th><th>Details</th></tr></thead><tbody>{reports.map((r:any)=><tr key={r.id}><td>{String(r.completion_date).slice(0,10)}</td><td className="font-bold">{r.group?.group_code}</td><td>{r.society?.name}</td><td>{r.family?.head_name??r.family_name??'-'}</td><td>{r.families_visited}</td><td>{r.families_completed}</td><td>{r.mobile??'-'}</td><td className="max-w-md whitespace-pre-wrap">{r.family_details}</td></tr>)}</tbody></table></div></section>
  </div>
 </AppLayout>;
}
