...
export default function Areas({centers}:Props){
 ...
 useEffect(()=>{
  if(recordSearch.trim()&&records.length===1&&!recordId) setRecordId(String(records[0].id));
 },[recordSearch,records,recordId]);
 useEffect(()=>{
  if(areaSearch.trim()&&areas.length===1&&!areaId) setAreaId(String(areas[0].id));
 },[areaSearch,areas,areaId]);
 const center=centers.find(c=>String(c.id)===centerId);

 return <AppLayout title="Sampark Area & Society"><Head title="Area & Society"/><div className="mb-5 rounded-2xl border border-[#dfcfe6] bg-white p-4 text-sm text-[#5b4862]"><b className="text-[#3e174e]">Where to assign Area / Society:</b> choose Center → Record Type (Group, Karyakar or Family) → Record → Sampark Area → Society → enter a reason → Save Assignment. If the Area/Society master does not exist yet, create it in <b>Settings / Master Data</b> using a role with that permission.</div><div className="grid gap-5 lg:grid-cols-[.7fr_1.3fr]">
   <section className="hf-card p-5"><h2 className="font-extrabold text-lg">Assign / Change</h2><p className="text-sm text-[#76647e] mb-4">Group, Karyakar and Family Area/Society changes require a reason and are written to Activity/Audit Logs.</p>{loadError&&<div className="mb-3 rounded-lg bg-red-50 p-3 text-sm text-red-700">{loadError}</div>}{areaSearch.trim()&&areas.length===0&&<div className="mb-3 rounded-lg bg-amber-50 p-3 text-xs font-semibold text-amber-800">No matching Sampark Area master found. Create the Area in Settings / Master Data first, then return here to assign it.</div>}<Form action="/assignments/areas" method="post" className="space-y-3">{({errors,processing})=><><FormField label="Center"><select className="hf-input" value={centerId} onChange={e=>{setCenterId(e.target.value);setRecordId('');setAreaId('');setRecordSearch('')}}>{centers.map(c=><option key={c.id} value={c.id}>{c.code} - {c.name}</option>)}</select></FormField><FormField label="Record Type"><select name="record_type" className="hf-input" value={type} onChange={e=>{setType(e.target.value);setRecordId('');setRecordSearch('');setAreaId('')}}><option value="group">Group</option><option value="karyakar">Karyakar</option><option value="family">Sankalp Family</option></select></FormField><FormField label="Find Record (filters the list below)"><input className="hf-input" value={recordSearch} onChange={e=>setRecordSearch(e.target.value)} placeholder={`Search ${type}`}/></FormField><FormField label="Choose Record" error={errors.record_id}><select name="record_id" className="hf-input" value={recordId} onChange={e=>setRecordId(e.target.value)} required><option value="">Select</option>{records.map(r=><option key={r.id} value={r.id}>{r.label}</option>)}</select></FormField><FormField label="Find Area (filters the list below)"><input className="hf-input" value={areaSearch} onChange={e=>setAreaSearch(e.target.value)} placeholder="Search area"/></FormField><FormField label="Choose Sampark Area" error={errors.sampark_area_id}><select name="sampark_area_id" className="hf-input" value={areaId} onChange={e=>setAreaId(e.target.value)} required><option value="">Select Area</option>{areas.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></FormField><FormField label="Society"><select name="society_id" className="hf-input"><option value="">No Society</option>{societies.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></FormField><FormField label="Reason / Change Note" error={errors.reason}><textarea name="reason" className="hf-input" rows={3} required/></FormField><button className="hf-btn w-full" disabled={processing}>Save Assignment</button></>}</Form></section>
