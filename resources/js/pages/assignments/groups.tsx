...
export default function Groups({groups,centers,karyakars,groupTypes,canCreate,filters}:Props) {
    ...
    const [candidates,setCandidates] = useState<Karyakar[]>(karyakars);
    useEffect(()=>{
        if(!centerId||!canCreate){setCandidates([]);return;}
        ...
        const timer=window.setTimeout(async()=>{
            try{
                ...
                const response=await fetch(`/assignments/groups/karyakars/search?${params.toString()}`,{headers:{Accept:'application/json'},credentials:'same-origin',signal:controller.signal});
                if(!response.ok)throw new Error('Karyakar search failed');
                const body=await response.json();
                setCandidates(Array.isArray(body.results)?body.results:[]);
            }catch(error:any){if(error?.name!=='AbortError')setCandidates([]);}
        },250);
        return()=>{window.clearTimeout(timer);controller.abort();};
    },[centerId,candidateSearch,canCreate]);
    ...
    return <AppLayout title="Group Management"><Head title="Group Management" />
        <div className="space-y-5">
            <section className="hf-card p-5 overflow-x-auto">
                ...
                <LiveFilterForm action="/assignments/groups" className="grid gap-2 md:grid-cols-4 mb-4">
                    <input name="search" className="hf-input" placeholder="Search Group code" defaultValue={filters.search??''}/>
            ...
            {canCreate&&<section className="hf-card p-5"><h2 className="text-lg font-extrabold">Create Group - exactly 2 Karyakars</h2><p className="text-sm text-[#76647e] mb-4">Group code is generated automatically from Center Code, for example GND-001.</p>
                <Form action="/assignments/groups" method="post" className="grid gap-4 lg:grid-cols-[.7fr_.8fr_1.5fr]">{({errors,processing})=><>
                    <FormField label="Center" error={errors.center_id}><select name="center_id" className="hf-input" value={centerId} onChange={e=>{setCenterId(e.target.value);setSelected([]);setCandidateSearch('')}} required>{centers.map(c=><option key={c.id} value={c.id}>{c.code} - {c.name}</option>)}</select></FormField>
                    ...
                    <div><div className="hf-label">Approved Karyakars - select exactly 2</div><input className="hf-input mb-2" value={candidateSearch} onChange={e=>setCandidateSearch(e.target.value)} placeholder="Search Karyakar name / reference"/><div className="max-h-56 overflow-auto rounded-xl border border-[#eadff0] p-2 space-y-1">{candidates.length===0?<div className="text-sm text-[#76647e] p-2">No matching approved Karyakars for this Center.</div>:candidates.map(k=><label key={k.id} className="flex items-center gap-2 rounded-lg p-2 hover:bg-[#faf5fc]"><input type="checkbox" checked={selected.includes(String(k.id))} onChange={()=>toggle(k.id)}/><span className="text-sm"><b>{k.full_name}</b> <span className="capitalize">({k.gender})</span><span className="block text-xs text-[#76647e]">{k.karyakar_reference} · {k.category}</span></span></label>)}</div><div className="mt-1 text-xs text-[#76647e]">Search returns up to 75 matching approved Karyakars instead of loading the full Center list.</div>{selected.map(id=><input key={id} type="hidden" name="karyakar_ids[]" value={id}/>)}{errors.karyakar_ids&&<div className="hf-error">{errors.karyakar_ids}</div>}</div>
