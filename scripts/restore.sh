...
if [ -f "$backup/storage-app.tar.gz" ]; then
  ...
  docker compose exec -T web sh -c 'mkdir -p /var/www/html/storage/app/public /var/www/html/storage/app/private && rm -rf /var/www/html/storage/app/public/* /var/www/html/storage/app/private/*'
  cat "$backup/storage-app.tar.gz" | docker compose exec -T web tar -C /var/www/html/storage/app -xzf -
else
  echo "Restoring legacy public-only storage backup..."
  docker compose exec -T web sh -c 'mkdir -p /var/www/html/storage/app/public && rm -rf /var/www/html/storage/app/public/*'
  cat "$backup/public-storage.tar.gz" | docker compose exec -T web tar -C /var/www/html/storage/app/public -xzf -
fi
